...
export default function InteractiveMap({
  ...
  return (
    <div className={`relative w-full ${height} rounded-3xl overflow-hidden bg-slate-950 border border-slate-800 shadow-elevated flex flex-col justify-between ${className}`}>
      {/* Map Header Overlay Bar */}
      <div className="absolute top-3.5 left-3.5 right-3.5 z-20 flex items-center justify-between pointer-events-none">
        <div className="pointer-events-auto bg-slate-900/90 backdrop-blur-md px-3.5 py-1.5 rounded-xl border border-slate-800 text-xs text-slate-200 flex items-center gap-2 font-bold shadow-sm">
          <MapPin className="w-3.5 h-3.5 text-rose-500" />
          <span>
            {mode === "tracking"
              ? "Live GPS Telemetry • Pro En Route"
              : mode === "heatmap"
              ? "Bangalore Urban Demand Density (Cooperative Heatmap)"
              : mode === "batch_route"
              ? "RWA Neighborhood Aggregation • Optimized Multi-Stop Route"
              : "Locality Cooperative Pro Network"}
          </span>
        </div>

        {mode === "tracking" && (
          <div className="pointer-events-auto bg-slate-900 text-white border border-slate-700 px-3.5 py-1.5 rounded-xl font-bold text-xs flex items-center gap-1.5 shadow-md">
            <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
            <span className="font-mono">ETA: {Math.max(2, Math.round((1 - progress) * 16))} Mins</span>
          </div>
        )}
      </div>
      ...
      <div className="relative w-full h-full bg-slate-950 overflow-hidden">
        <svg className="w-full h-full opacity-95" viewBox="0 0 650 450">
          ...
          <pattern id="city-grid" width="36" height="36" patternUnits="userSpaceOnUse">
            <path d="M 36 0 L 0 0 0 36" fill="none" stroke="#1e293b" strokeWidth="0.75" />
          </pattern>
          <rect width="100%" height="100%" fill="url(#city-grid)" />

          {/* Primary Arterial Road Networks */}
          <path d="M 40 210 Q 300 170 610 270" fill="none" stroke="#1e293b" strokeWidth="10" />
          <path d="M 230 40 L 230 410" fill="none" stroke="#1e293b" strokeWidth="7" />
          <path d="M 410 30 L 410 420" fill="none" stroke="#1e293b" strokeWidth="7" strokeDasharray="6 4" />
          <path d="M 100 380 Q 250 250 450 160" fill="none" stroke="#334155" strokeWidth="5" strokeDasharray="8 4" />
          ...
          {mode === "heatmap" && (
            <g>
              ...
              <text x="240" y="195" fill="#94a3b8" fontSize="9" textAnchor="middle">
                42 Bookings Today
              </text>
              ...
              <text x="220" y="320" fill="#e2e8f0" fontSize="10" fontWeight="semibold" textAnchor="middle">
                HSR Layout Sector 3
              </text>

              {/* Whitefield Medium Hotspot */}
              <circle cx="520" cy="280" r="85" fill="url(#heat-steady)" />
              <text x="520" y="280" fill="#e2e8f0" fontSize="10" fontWeight="semibold" textAnchor="middle">
            ...
            <g>
              ...
              {[
                ...
                <g key={i} transform={`translate(${h.x}, ${h.y})`}>
                  ...
                  <rect x="-38" y="16" width="76" height="18" rx="5" fill="#0f172a" stroke="#334155" />
                  <text y="28" fill="#e2e8f0" fontSize="8" textAnchor="middle">
                    {h.label}
                  </text>
                </g>
              ))}
        ...
        {mode === "pins" &&
          pins.map((p) => (
            <motion.button
              ...
              className="absolute -translate-x-1/2 -translate-y-1/2 z-10 focus:outline-none cursor-pointer"
            >
              <div
        ...
        {mode === "tracking" && (
          ...
          >
            <div className="relative flex flex-col items-center">
              <div className="w-11 h-11 rounded-full border-2 border-emerald-500 bg-slate-900 p-0.5 shadow-elevated flex items-center justify-center">
                <img
              ...
              <div className="mt-1 bg-slate-900 text-white text-[9px] font-bold px-2 py-0.5 rounded-md border border-slate-700 whitespace-nowrap shadow-sm">
                {activeWorker.name} (15m Dispatch)
              </div>
            </div>
          </motion.div>
        )}
      ...
      {mode === "pins" && selectedPinWorker && (
        <div className="absolute bottom-3.5 left-3.5 right-3.5 bg-slate-900/95 backdrop-blur-md p-3.5 rounded-2xl border border-slate-800 text-white flex items-center justify-between gap-4 z-30 shadow-elevated">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="text-xs font-bold flex items-center gap-2">
                ...
                <span className="bg-slate-800 text-slate-300 border border-slate-700 px-2 py-0.2 rounded-full text-[10px]">
